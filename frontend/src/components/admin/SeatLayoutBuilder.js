...
const SeatLayoutBuilder = ({ show, onHide, onSave, initialLayout = [] }) => {
  ...
  const addSeat = (rowIndex) => {
    ...
    if (row.seats.length >= 30) {
      alert('Maximum 30 seats per row allowed');
      return;
    }
  ...
  const addGap = (rowIndex) => {
    ...
    if (newLayout[rowIndex].seats.length >= 30) {
      alert('Maximum 30 seats per row allowed');
      return;
    }
  ...
  const removeSeat = (rowIndex, seatIndex) => {
    const newLayout = [...layout];
    newLayout[rowIndex].seats.splice(seatIndex, 1);
    setLayout(newLayout);
  };
  ...
  const toggleSeatGap = (rowIndex, seatIndex) => {
    ...
    if (currentSeat === null) {
      // Convert gap to seat
      const maxSeat = Math.max(...newLayout[rowIndex].seats.filter(s => s !== null));
  ...
  const renderSeatLayoutBuilder = () => (
    <div className="seat-layout-builder">
      ...
      {layout.map((rowObj, rowIndex) => (
        ...
        >
          <div className="d-flex align-items-center mb-2">
            {rowObj.seats.length > 0 && <Badge bg="primary" className="me-2">{rowObj.row}</Badge>}
            {rowObj.seats.length === 0 && <Badge bg="secondary" className="me-2">Empty Row</Badge>}
            <Button 
      ...
      <div className="text-center mt-4">
        <div className="mb-3">
          ...
          <div className="d-flex flex-wrap justify-content-center gap-2">
            <Button variant="outline-primary" size="sm" onClick={() => addCustomRow('left-right')}>
              Left-Gap-Right
            </Button>
            <Button variant="outline-success" size="sm" onClick={() => addCustomRow('middle-only')}>
              Middle Only
            </Button>
            <Button variant="outline-warning" size="sm" onClick={() => addCustomRow('full-row')}>
              Full Row
            </Button>
            <Button variant="outline-info" size="sm" onClick={() => addCustomRow('random-gaps')}>
              Random Gaps
            </Button>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-center gap-2">
          <Button variant="outline-primary" onClick={addRow}>
            <FaPlus className="me-1" />Add Custom Row
          </Button>
      ...
      <div className="mt-4 p-3 rounded" style={{ background: 'rgba(255,255,255,0.05)' }}>
        ...
        <p className="text-secondary mb-1">Total Capacity: {calculateTotalCapacity()}</p>
  ...
  return (
    <Modal show={show} onHide={onHide} size="xl" centered>
      <Modal.Header closeButton className="border-0" style={{ background: '#1f2025' }}>
        <Modal.Title className="text-white">
          {previewMode ? 'Seat Layout Preview' : 'Custom Seat Layout Builder'}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body style={{ background: '#1f2025', maxHeight: '70vh', overflowY: 'auto' }}>
