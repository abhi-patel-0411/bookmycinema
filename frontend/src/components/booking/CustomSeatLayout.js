...
const CustomSeatLayout = ({ 
  ...
  if (!seatLayout || seatLayout.length === 0) {
    return <div className="text-center text-secondary py-4">No seat layout available</div>;
  }
  ...
  return (
    <div className="seat-layout-professional">
      ...
      <div className="seat-layout-scroll" ref={scrollContainerRef} style={{ marginTop: '30px', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        {seatLayout.map((rowObj, rowIndex) => {
          const seatClass = getSeatClass(rowObj, rowIndex);
          const sectionLabel = rowObj.section || (seatClass.charAt(0).toUpperCase() + seatClass.slice(1));
              ...
              >
                ...
                <div className={`seats-container-professional ${rowObj.seats.filter(seat => seat !== null).length <= 15 ? 'seats-centered' : ''}`}>
                  {rowObj.seats.map((seat, seatIndex) => {
                    ...
                    else if (status === 'selected') seatClassName += 'seat-selected';
                    else if (status === 'locked') seatClassName += 'seat-locked';
                    else if (seatClass === 'premium') seatClassName += 'seat-premium';
                    else if (seatClass === 'gold') seatClassName += 'gold-seat';
                    else if (seatClass === 'silver') seatClassName += 'silver-seat';
                    else seatClassName += 'seat-available';

                    return (
                      <button
                        key={seatIndex}
                        onClick={() => handleSeatClick(rowObj.row, seat, rowIndex, seatClass)}
                        disabled={status === 'booked' || status === 'locked'}
                        title={`${rowObj.row}${seat} - ₹${price} (${seatClass.toUpperCase()}) ${status === 'locked' ? '- Being selected by another user' : status === 'selected' ? '- Click to deselect' : ''}`}
                        className={seatClassName}
                      >
                        {status === 'booked' ? '✕' : status === 'locked' ? '⏳' : seat}
                      </button>
                    );
                  })}
                </div>
