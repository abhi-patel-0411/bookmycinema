...
const TheaterListLayout = ({ theaters, onMovieClick }) => {
  const generateShowTimes = (theater) => {
    ...
    const formats = theater.screens?.map(screen => screen.screenType || '2D') || ['2D', 'IMAX', '3D'];
  ...
  const getTheaterIcon = (theaterName) => {
    ...
    if (name.includes('pvr')) return 'https://assets-in.bmscdn.com/moviesmaster/movies-showtimes/v4/cinema-icon/pvr.png';
    if (name.includes('inox')) return 'https://assets-in.bmscdn.com/moviesmaster/movies-showtimes/v4/cinema-icon/inox.png';
    if (name.includes('cinepolis')) return 'https://assets-in.bmscdn.com/moviesmaster/movies-showtimes/v4/cinema-icon/cinepolis.png';
              ...
              <div className="theater-amenities">
                <div className="amenities-list">
                  ...
                  {theater.amenities.length > 3 && (
                    <span className="amenity-tag more">+{theater.amenities.length - 3}</span>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="active-movies">
                ...
                theater.movies.slice(0, 3).map((movie, idx) => (
                  ...
                  >
                    <div className="movie-slot-info">
                      ...
                      <div className="movie-screen">Screen {movie.shows[0]?.screenNumber || idx + 1}</div>
                    </div>
                  </div>
                ))
              ) : (
              ...
              {theater.movies && theater.movies.length > 3 && (
                <div className="movie-slot more-slot">
                  <div className="movie-slot-info">
                    <div className="movie-name">+{theater.movies.length - 3}</div>
            ...
            <div className="theater-policies">
              <div className="policy-info">
                <div className="policy-text">
                  <div className="cancellation-policy">Cancellation available</div>
                </div>
              </div>
            </div>
          </div>
