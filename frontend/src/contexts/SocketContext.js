  ...
  useEffect(() => {
    const socketInstance = io(process.env.REACT_APP_API_URL?.replace('/api', '') || 
      (window.location.hostname === 'localhost' 
        ? 'http://localhost:5000' 
        : 'https://bookmycinema-backend.onrender.com'));
    ...
    socketInstance.on("seat-conflict", (data) => {
      ...
      if (data.userId === userId) {
        window.dispatchEvent(
          new CustomEvent("seat-conflict", { detail: data })
        );
    ...
    socketInstance.on("seats-auto-released", (data) => {
      ...
      if (!data.userId || data.userId === userId) {
        window.dispatchEvent(
          new CustomEvent("seats-auto-released", { detail: data })
        );
    ...
    socketInstance.on("seats-available", (data) => {
      window.dispatchEvent(
        new CustomEvent("seats-available", { detail: data })
      );
    });
    ...
    socketInstance.on("seat-status-sync", (data) => {
      window.dispatchEvent(new CustomEvent("seat-status-sync", { detail: data }));
      console.log("Seat status synced:", data);
    });

    socketInstance.on("seats-cleanup", (data) => {
      window.dispatchEvent(new CustomEvent("seats-cleanup", { detail: data }));
      console.log("Seats cleaned up:", data);
    });

    // Request current seat status on connect to handle page refresh
    socketInstance.on("connect", () => {
      setTimeout(() => {
        socketInstance.emit('request-seat-status');
      }, 1000);
    });
