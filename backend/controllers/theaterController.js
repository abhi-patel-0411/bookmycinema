...
const createTheater = async (req, res) => {
  try {
    ...
    if (theaterData.amenities && typeof theaterData.amenities === "string") {
      theaterData.amenities = theaterData.amenities
        .split(",")
        .map((a) => a.trim());
    }
...
const updateTheater = async (req, res) => {
  try {
    ...
    const theater = await Theater.findByIdAndUpdate(req.params.id, updateData, {
      new: true,
      runValidators: true,
    });
...
const addScreen = async (req, res) => {
  try {
    ...
    if (seatLayout && seatLayout.length > 0) {
      ...
      actualCapacity = seatLayout.reduce(
        (total, row) =>
          total + row.seats.filter((seat) => seat !== null).length,
        0
      );
    } else {
      ...
      actualCapacity = finalSeatLayout.reduce(
        (total, row) =>
          total + row.seats.filter((seat) => seat !== null).length,
        0
      );
    ...
    const theatersWithShows = await Promise.all(
      theaters.map(async (theater) => {
        const shows = await Show.find({
          ...
          .lean();
        //Return plain JavaScript objects, not full Mongoose documents.
        const movieMap = {};
        shows.forEach((show) => {
          if (show.movie) {
...
const getTheaterAnalytics = async (req, res) => {
  try {
    const analytics = await Theater.aggregate([
      {
        $match: {
          $or: [{ "status.isActive": true }, { isActive: true }],
        },
      },
      {
        $group: {
...
const bulkUpdateTheaters = async (req, res) => {
  try {
    ...
    res.json({
      message: `${result.modifiedCount} theaters updated successfully`,
    });
  } catch (error) {
...
const updateSeatLayout = async (req, res) => {
  try {
    ...
    const newCapacity = seatLayout.reduce(
