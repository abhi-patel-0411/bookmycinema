...
const updateComingSoonMovies = async () => {
  try {
    ...
    for (const movie of movies) {
      ...
      const needsUpdate =
        movie.isUpcoming !== shouldBeUpcoming ||
      ...
      if (needsUpdate) {
        ...
        updatedCount++;
        console.log(
          `Updated ${movie.title}: upcoming ${
            movie.isUpcoming
          } -> ${shouldBeUpcoming}${!shouldBeActive ? ", deactivated" : ""}`
        );
  ...
  try {
    ...
    if (movieData.startDate) {
      ...
      if (startDate > currentDate) {
        ...
        console.log(
          "Movie set as now showing - showing starts today or already started"
        );
      }
    } else {
      movieData.isUpcoming =
        movieData.isUpcoming === "true" || movieData.isUpcoming === true;
    }
...
const updateMovie = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      updateData.poster &&
      (updateData.poster.startsWith("http://") ||
        updateData.poster.startsWith("https://"))
    ) {
    ...
    if (updateData.startDate) {
      ...
      if (startDate > currentDate) {
        ...
        console.log(
          "Movie updated as now showing - showing starts today or already started"
        );
      }
    } else {
      updateData.isUpcoming =
        updateData.isUpcoming === "true" || updateData.isUpcoming === true;
    }
    ...
    if (updateData.startDate || updateData.endDate) {
      ...
      console.log(
        `Movie ${finalMovie.title} updated: isUpcoming = ${finalMovie.isUpcoming}`
      );
...
const deleteMovie = async (req, res) => {
  try {
    ...
    console.log(
      `Attempting to delete movie ${req.params.id} and all related data`
    );
    ...
    try {
      ...
      if (showIds.length > 0) {
        ...
        console.log(
          `Updated ${bookingUpdateResult.modifiedCount} bookings to cancelled status`
        );
      }
      ...
      console.log(
        `Deleted ${showDeleteResult.deletedCount} shows for movie ${req.params.id}`
      );
    } catch (deleteError) {
      console.error(
        `Error deleting related data for movie ${req.params.id}:`,
        deleteError
      );
...
const softDeleteMovie = async (req, res) => {
  try {
    ...
    try {
      const showDeleteResult = await Show.deleteMany({ movie: req.params.id });
      console.log(
        `Deleted ${showDeleteResult.deletedCount} shows for inactive movie ${req.params.id}`
      );
    } catch (deleteError) {
      console.error(
        `Error deleting shows for movie ${req.params.id}:`,
